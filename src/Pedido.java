...
public class Pedido {
	...
	public boolean incluirProduto(Produto novo) {
		boolean pedidoCadastrado = verificarProdutoNoCarrinho(novo);
		if (pedidoCadastrado){
			return false;
		}

		if (quantProdutos < MAX_PRODUTOS && novo.getQuantidadeEmEstoque() > 0) {
			produtos[quantProdutos++] = novo;
			novo.setQuantidadeEmEstoque(quantProdutos--);
			return true;
		}
		return false;
	}

	public boolean verificarProdutoNoCarrinho(Produto solicitacao){
		for (Produto produto : produtos){
			if (produto.equals(solicitacao)){
				return true;
			}
		}
